...
export default function Shop({initialProducts}:{initialProducts:Product[]}) {
 ...
 async function order(){
   ...
   const res=await fetch("/api/orders",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
    customer, configuration:config, totalCents:Math.round(price), productId:selected.id, quantity:1
   })});
 ...
 return <main>
  ...
  <section id="konfigurator" className="section config"><div className="wrap"><div className="center"><small>KONFIGURATOR</small><h2>Ihre Schließanlage</h2></div>
   ...
   {step===3&&<Panel title="Schließplan"><p>Beispielhafte Berechtigungsmatrix. Im nächsten Ausbau können Türen und Nutzer frei hinzugefügt werden.</p><div className="table"><table><thead><tr><th>Tür</th><th>Geschäftsführung</th><th>Büro</th><th>Lager</th><th>Technik</th></tr></thead><tbody>{["Haupteingang","Büro 1","Lager","Technikraum"].map((t,i)=><tr key={t}><td>{t}</td>{[0,1,2,3].map(j=><td key={j}><input type="checkbox" defaultChecked={i===0||j===i}/></td>)}</tr>)}</tbody></table></div></Panel>}
